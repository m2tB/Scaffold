...
var HttpServiceSrvHandler *http.Server

func HttpServiceServerRun() {
	// 初始默认中间件
	...
	router := iroutes.ServiceRoute(middleware...)
	HttpServiceSrvHandler = &http.Server{
	...
	go func() {
		...
		if err := HttpServiceSrvHandler.ListenAndServe(); err != nil && err != http.ErrServerClosed {
			zap.L().Debug(fmt.Sprintf("项目启动失败, 启动端口: %s, err: %v\n", istruct.Conf.HttpListenPort, err))
		}
	}()
}

func HttpServiceServerStop() {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	if err := HttpServiceSrvHandler.Shutdown(ctx); err != nil {
